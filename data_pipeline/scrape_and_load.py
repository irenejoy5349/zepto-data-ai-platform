...
def main():
    ...
    # -------------------------------------------------
    # QUERY 3: DISTINCT
    # Category names are stored in the categories table
    # -------------------------------------------------
    query_3 = """
    ...
    # -------------------------------------------------
    print("\n" + "=" * 70)
    print("TASK 6A - pd.read_sql() RESULTS")
    print("=" * 70)
    ...
    print("\nResult from pd.read_sql() - Query 1:")
    print(sql_df_1.to_string(index=False))

    print("\nResult from pd.read_sql() - JOIN Query:")
    print(sql_df_2.to_string(index=False))

    # -------------------------------------------------
    # TASK 6B: Reproduce the JOIN using pd.merge()
    # -------------------------------------------------
    ...
    merge_df = merge_df.sort_values(
        by=["rating", "category_name", "title"],
        ascending=[False, True, True]
    ).reset_index(drop=True)

    sql_join_sorted = sql_df_2.sort_values(
        by=["rating", "category_name", "title"],
        ascending=[False, True, True]
    ).reset_index(drop=True)

    print("\n" + "=" * 70)
    print("TASK 6B - JOIN COMPARISON")
    print("=" * 70)

    print("\nSQL JOIN result:")
    print(sql_join_sorted.to_string(index=False))

    print("\npd.merge() result:")
    print(merge_df.to_string(index=False))

    # Check whether both approaches are equivalent
    equivalent = sql_join_sorted.equals(merge_df)

    print(
        f"\nDo SQL JOIN and pd.merge() produce equivalent output? "
        f"{equivalent}"
    )

    if equivalent:
        print("SUCCESS: Both approaches produce the same result.")
    else:
        print(
            "WARNING: Results are not identical. "
            "Check sorting or data types."
        )
